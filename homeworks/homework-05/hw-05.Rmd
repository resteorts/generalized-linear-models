...
logistic_Newton_Raphson <- function(x, y, b.init, tol = 1e-8) {
  ...
  while (change > tol) {
    ...
    mu <- as.vector(1 / (1 + exp(-eta)))     # Logistic mean
    w <- diag(mu * (1 - mu))                 # Weight matrix
    z <- eta + (y - mu) / (mu * (1 - mu))    # Adjusted response (optional, for IRLS)
    
    # Newton-Raphson update
    b.new <- b.old + solve(t(x) %*% w %*% x) %*% t(x) %*% (y - mu)
...
logistic_Newton_Raphson(x, y, b.init = c(1,1,1))
```
