...
\[
f(y) = \frac{\alpha x_m^\alpha}{y^{\alpha+1}} \mathbf{1}_{\{y \geq x_m\}}
\]

where:
\begin{itemize}
    \item \( \mathbf{1}_{\{y \geq x_m\}} \) is the indicator function, which is 1 if \( y \geq x_m \) and 0 otherwise.
    \item \( \alpha > 0 \) is the shape parameter.
    \item \( x_m > 0 \) is the scale parameter of the data.
\end{itemize}
