...
- In later classes, STA 402, you will consider how to estimate $\theta$ when it's random
...
Suppose $Y = \sum_{i=1}^n X_i.$ Then $Y \sim Binomial(n, \theta)$.\footnote{This represents $n$ Bernoulli trials with success probability $\theta.$}
...
$$X_1,\ldots,X_n \stackrel{iid}{\sim} \Bernoulli(\theta).$$
...
\begin{equation}
\label{eqn:mle}
\frac{\partial \log p(x_{1:n} \mid \theta)}{\partial \theta} = 0.
\end{equation}
...
Thus, we need to ensure that we haven't found a local one. (Show that the second derivative is $< 0$.)
...
Exercise: Derive (step by step) that the MLE for $\theta$ is $\frac{1}{n}\sum_i x_i = \bar{x}.$
...
\[
L(\theta)=\theta^{S}(1-\theta)^{n-S}, \qquad S=\sum_{i=1}^n x_i,\ \theta\in[0,1].
\]
...
For \(\theta\in(0,1)\), differentiate and set to zero:
\[
...
\]
$$\Longrightarrow\;
\hat\theta=\frac{S}{n}=\frac{1}{n}\sum_{i=1}^n x_i.
$$
...
- The log-likelihood is log-concave, which means the likelihood is concave (both functions are cap-shaped).
- This ensures that that solution is a global one (and we do not have to take the second derivative). 
...
- The Beta distribution is often used as a prior for the probability of success in Bernoulli or Binomial experiments in Bayesian statistics (STA 402). 
