...
## Coefficient interpretation for GLMs

For a predictor \(X_j\),
\[
\beta_j = \frac{\partial}{\partial X_j}
\Big[ g\!\left( \mathbb{E}[Y \mid X] \right) \Big],
\]
that is, \(\beta_j\) is the additive change in the \emph{link-transformed mean (or median)} associated with a one-unit increase in \(X_j\), holding other variables fixed.

To interpret \(\beta_j\) on the response scale, apply the inverse link:
\[
