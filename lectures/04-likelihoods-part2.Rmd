...
$$BIC = -2 (max \text{log-likelihood}) + p log(n)$$
...
- Our findings from the nested and non-nested test support the more simple model. 

\vspace*{1em}

<!-- - Note that the sample size is quite low $(n=30)$, and it appears that the LRT has low power, which  -->
<!-- is likely why it prefer the more simple model -->
<!-- - In contrast, while BIC’s penalty is fairly mild, a modest likelihood improvement can make BIC pick the larger model even though the LRT doesn’t reject the null. -->
<!-- - Those are consistent statements because model selection (BIC) and hypothesis testing (LRT) are different decisions.  -->
<!-- - How to proceed? Typically, keep the more simple model (unless you have strong other evidence to support the more complex model).  -->
