...
<!-- The goal of this exercise is to use simulation to understand the equivalency between a gamma-Poisson mixture and a negative binomial distribution empirically. -->
...
<!-- Use the R function rpois() to generate 10,000 $x_i$ from a regular Poisson distribution,  -->
<!-- $$X \sim \textrm{Poisson}(\lambda=1.5)$$.  -->

<!-- Plot a histogram of this distribution and note its mean and variance.  -->

<!-- Next, let $Y \sim \textrm{Gamma}(\text{shape} = 3, \text{rate} = 2)$ and use rgamma() to generate 10,000 random $y_i$ from this distribution. -->
...
<!-- set.seed(101) -->
<!-- n <- 10000 # sample size  -->
<!-- x <-  rpois(n, lambda = 1.5) # draw poisson data -->
<!-- y <- rgamma(n, shape = 3, rate = 2) # draw gamma data -->

<!-- z <- rep(0, n) -->
<!-- for(i in 1:n) -->
<!--   z[i] = rpois(1, lambda = y[i]) # what distribution does this generate? -->
<!-- ``` -->
...
<!-- ```{r} -->
<!-- par(mfrow=c(1,2)) -->
<!-- hist(x, breaks=30, main = "Histogram of x", freq=F)  -->
<!-- hist(z, breaks=30, main = "Histogram of z", freq=F) -->
<!-- ``` -->

<!-- ## Solution -->

<!-- ```{r} -->
<!-- print(paste0("mean of x ", signif(mean(x), 5))) -->
<!-- print(paste0("var of x ", var(x) %>% signif(5))) -->
<!-- print(paste0("mean of z ", signif(mean(z), 5))) -->
<!-- print(paste0("var of z ", var(z) %>% signif(5))) -->
<!-- ``` -->

<!-- The means are the same (after rounding); the variance of $z$ is larger than $x$. It would be prudent to theoretically understand the relationship between the mean and variance for a more precise conclusion.  -->
...
<!-- Note that $\lambda$ is a rate parameter so that  -->

<!-- $$E[Y] = r/\lambda$$ for $$Y\sim \text{Gamma}(\text{shape} = r = 3, \text{rate} = \lambda = p/(1-p)).$$  -->

<!-- Observe that  -->
<!--     \begin{align} -->
<!--     Z_i &\mid Y_i \sim \text{Poisson}(\lambda = Y_i)\\ -->
<!--     Y_i &\sim \text{Gamma}\left(r = 3, \lambda = (2/3)/(1-2/3)\right)\\ -->
<!--     Z_i &\sim \text{Negbin}(r=3, p=2/3) -->
<!--     \end{align} -->

<!-- and $E[Z_i] = r(1-p)/p = 1.5 = E[X_i]$, $V[Z_i]= r(1-p)/p^2 = 9/4 = E[X_i]/p$, so the mean is the same and the variance is inflated by $1/p$. -->

<!-- Let $u_i\sim Negbin(r=3, p=2/3)$. We can see that they have the same distribution. -->

<!-- ## Solution -->

<!-- ```{r} -->
<!-- u = rnbinom(n, size = 3, prob=2/3) -->
<!-- par(mfrow=c(1,2)) -->
<!-- hist(z, breaks=30, main = "Histogram of z", freq=F) -->
<!-- hist(u, breaks=30, main = "Histogram of u", freq=F) -->
<!-- par(mfrow=c(1,1)) -->
<!-- ``` -->
...
<!-- print(paste0("mean of z ", signif(mean(z), 5))) -->
<!-- print(paste0("var of z ", var(z) %>% signif(5))) -->
<!-- print(paste0("mean of u ", signif(mean(u), 5))) -->
<!-- print(paste0("var of u ", var(u) %>% signif(5))) -->
<!-- print("summary of z") -->
<!-- summary(z) -->
<!-- print("summary of u") -->
<!-- summary(u) -->
