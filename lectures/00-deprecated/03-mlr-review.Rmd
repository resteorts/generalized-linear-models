...
- $y_{ij}$ are the observed data. 
- $\epsilon_{ij}$ is random.
- $\beta_0$, $\beta_1$, and $x_i$ are deterministic, fixed.

$E[y_{ij}] = \beta_0 + \beta_1x_i$
...
Today's data is from the Kentucky Derby, an annual 1.25-mile horse race held at the Churchill Downs race track in Louisville, KY. The data is in the file [data/derbyplus.csv](data/derbyplus.csv) and contains information for races 1896 - 2017.
...
```{r univar-eda-plot, echo = F, warning=FALSE, message = F}
...
p2 <- ggplot(data = derby, aes(x = starters)) + 
  ...
  labs(x = "Number of Starters", y = "Count")
...
```{r bivar-eda-plot, echo = F, warning=FALSE, echo = F}
p4 <- ggplot(data = derby, aes(x = starters, y = speed)) + 
  geom_point() + 
  labs(x = "Number of Starters", y = "Speed (ft / s)")
...
```{r scatterplot-matrix-plot, echo = F, fig.width = 10, warning=FALSE}
#library(GGally)
ggpairs(data = derby, 
...
```{r scatterplot-matrix, eval = F, warning=FALSE, message=F}
library(GGally)
ggpairs(data = derby, 
...
```{r multivar-eda-plot, echo = F, warning=FALSE, message = F}
library(viridis)
